...
public:
    explicit getter(
        Class& owner,
        const T value,
        const func getter = [](Class&, const T& value) -> const T { return value; },
...
public:
    explicit getset(
        Class& owner,
        const T value,
        const func setter = [](Class&, const T& value) -> const T { return value; },
        const func getter = [](Class&, const T& value) -> const T { return value; },
        const del deleter = [](Class&, T& value) {})
...
public:
    ...
    zip_iterator(T&... containter) : containters(std::forward_as_tuple(containter...))
    {
    ...
    iterator end()
    {
        return std::apply([](auto&... container) { return iterator(container.end()...); },
                          containters);
    }
...
public:
    ...
    const_zip_iterator(const T&... containter) : containters(std::forward_as_tuple(containter...))
    {
        static_assert(sizeof...(T) > 0, "At least one container is required for zipping.");
        const auto all_equal = [](auto const& c0, auto const&... c) {
            return ((c0.size() == c.size()) && ...);
    ...
    iterator end()
    {
        return std::apply([](auto&... container) { return iterator(container.cend()...); },
                          containters);
    }
...
public:
    ...
    enumerated_zip_iterator(T&... containter) : containters(std::forward_as_tuple(containter...))
    {
        static_assert(sizeof...(T) > 0, "At least one container is required for zipping.");
        const auto all_equal = [](auto const& c0, auto const&... c) {
            return ((c0.size() == c.size()) && ...);
    ...
    {
        return std::apply(
            [](auto&... container) { return iterator(size_t{0}, container.begin()...); },
            containters);
    }
...
public:
    ...
    class iterator
        : public std::iterator<
              ...
              std::tuple<size_t, typename T::const_iterator...>,         // difference_type
              const std::tuple<size_t, typename T::const_iterator...>*,  // pointer
    ...
    {
        return std::apply(
            [](auto&... container) { return iterator(size_t{0}, container.cbegin()...); },
            containters);
    }
...
std::ostream& operator<<(std::ostream& os, const std::map<K, V>& arr)
{
    std::cout << '{';
    for (auto i = arr.begin(); i != arr.end(); ++i)
        std::cout << (i == arr.begin() ? "" : ", ") << i->first << ": " << i->second;
    std::cout << '}';
    return os;
}
...
template <class K, class V>
std::ostream& operator<<(std::ostream& os, const std::unordered_map<K, V>& arr)
{
    std::cout << '{';
    for (auto i = arr.begin(); i != arr.end(); ++i)
        std::cout << (i == arr.begin() ? "" : ", ") << i->first << ": " << i->second;
